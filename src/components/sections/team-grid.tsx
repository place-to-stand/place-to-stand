'use client'

import { useEffect, useState } from 'react'
import Image from 'next/image'
import { X } from 'lucide-react'
import { team, type TeamMember } from '@/src/lib/team'
import { Button } from '@/src/components/ui/button'
import { cn } from '@/src/lib/utils'

function SocialLinks({ member }: { member: TeamMember }) {
  if (!member.socials) return null
  return (
    <div className='flex items-center gap-3'>
      {member.socials.linkedin && (
        <a
          href={member.socials.linkedin}
          target='_blank'
          rel='noopener noreferrer'
          className='text-ink/50 transition-colors hover:text-ink'
          aria-label={`${member.name} on LinkedIn`}
        >
          <svg className='size-5' fill='currentColor' viewBox='0 0 24 24'>
            <path d='M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z' />
          </svg>
        </a>
      )}
      {member.socials.x && (
        <a
          href={member.socials.x}
          target='_blank'
          rel='noopener noreferrer'
          className='text-ink/50 transition-colors hover:text-ink'
          aria-label={`${member.name} on X`}
        >
          <svg className='size-5' fill='currentColor' viewBox='0 0 24 24'>
            <path d='M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z' />
          </svg>
        </a>
      )}
      {member.socials.instagram && (
        <a
          href={member.socials.instagram}
          target='_blank'
          rel='noopener noreferrer'
          className='text-ink/50 transition-colors hover:text-ink'
          aria-label={`${member.name} on Instagram`}
        >
          <svg className='size-5' fill='currentColor' viewBox='0 0 24 24'>
            <path d='M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z' />
          </svg>
        </a>
      )}
      {member.socials.github && (
        <a
          href={member.socials.github}
          target='_blank'
          rel='noopener noreferrer'
          className='text-ink/50 transition-colors hover:text-ink'
          aria-label={`${member.name} on GitHub`}
        >
          <svg className='size-5' fill='currentColor' viewBox='0 0 24 24'>
            <path d='M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z' />
          </svg>
        </a>
      )}
    </div>
  )
}

type TeamGridProps = {
  /**
   * When true, each card shows a 01/02/03-style index in the upper left,
   * mirroring the numbered profiles pattern used on the inspiration site.
   */
  numbered?: boolean
}

export function TeamGrid({ numbered = false }: TeamGridProps = {}) {
  const [activeMember, setActiveMember] = useState<TeamMember | null>(null)

  useEffect(() => {
    if (!activeMember) return
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setActiveMember(null)
    }
    window.addEventListener('keydown', handleKey)
    document.body.style.overflow = 'hidden'
    return () => {
      window.removeEventListener('keydown', handleKey)
      document.body.style.overflow = ''
    }
  }, [activeMember])

  return (
    <>
      <div className='mx-auto grid w-full max-w-6xl grid-cols-1 gap-6 md:grid-cols-3 md:gap-8'>
        {team.map((member, index) => (
          <article
            key={member.name}
            className='group flex flex-col gap-5 border border-ink/15 bg-white p-6 text-ink shadow-sm transition-all duration-300 hover:border-ink/40 hover:shadow-lg md:p-8'
          >
            <div className='relative aspect-[4/3] w-full overflow-hidden border border-ink/10'>
              <Image
                src={member.image}
                alt={`${member.name}, ${member.title}`}
                fill
                className='object-cover transition duration-700 group-hover:scale-105'
                sizes='(min-width: 768px) 33vw, 100vw'
              />
            </div>
            <div className='flex flex-col gap-1'>
              {numbered && (
                <span className='text-xs font-semibold uppercase tracking-[0.15em] text-ink/40'>
                  {String(index + 1).padStart(2, '0')}
                </span>
              )}
              <h3 className='font-headline text-xl uppercase leading-tight md:text-2xl'>
                {member.name}
              </h3>
              <p className='text-sm font-semibold uppercase tracking-[0.05em] text-ink/60'>
                {member.title}
              </p>
            </div>
            <p className='text-base leading-snug text-ink/80'>
              {member.shortBio}
            </p>
            <div className='mt-auto flex items-center justify-between gap-4 pt-2'>
              <SocialLinks member={member} />
              <Button
                size='sm'
                variant='outline'
                onClick={() => setActiveMember(member)}
              >
                Learn more →
              </Button>
            </div>
          </article>
        ))}
      </div>

      {/* Slide-out bio panel */}
      <div
        className={cn(
          'fixed inset-0 z-[100] transition-opacity duration-300',
          activeMember
            ? 'pointer-events-auto opacity-100'
            : 'pointer-events-none opacity-0'
        )}
        aria-hidden={!activeMember}
      >
        {/* Backdrop */}
        <div
          className='absolute inset-0 bg-black/50'
          onClick={() => setActiveMember(null)}
        />

        {/* Panel */}
        <aside
          className={cn(
            'absolute inset-y-0 right-0 flex w-full max-w-xl flex-col overflow-y-auto bg-white shadow-2xl transition-transform duration-300 ease-out',
            activeMember ? 'translate-x-0' : 'translate-x-full'
          )}
          role='dialog'
          aria-modal='true'
          aria-label={activeMember ? `${activeMember.name} bio` : undefined}
        >
          <button
            type='button'
            onClick={() => setActiveMember(null)}
            className='absolute right-4 top-4 z-10 rounded-full border border-ink/20 bg-white p-2 text-ink transition-colors hover:bg-ink hover:text-white'
            aria-label='Close'
          >
            <X className='size-5' />
          </button>

          {activeMember && (
            <div className='flex flex-col gap-6 p-8 pt-16 md:p-12 md:pt-16'>
              <div className='relative aspect-square w-40 overflow-hidden rounded-2xl border border-ink/10'>
                <Image
                  src={activeMember.image}
                  alt={activeMember.name}
                  fill
                  className='object-cover'
                  sizes='160px'
                />
              </div>
              <div className='flex flex-col gap-1'>
                <h2 className='font-headline text-3xl uppercase leading-tight text-ink md:text-4xl'>
                  {activeMember.name}
                </h2>
                <p className='text-sm font-semibold uppercase tracking-[0.05em] text-ink/60'>
                  {activeMember.title}
                </p>
              </div>
              <SocialLinks member={activeMember} />
              <div className='flex flex-col gap-4 text-base leading-relaxed text-ink/80'>
                {activeMember.longBio.split('\n\n').map((paragraph, i) => (
                  <p key={i}>{paragraph}</p>
                ))}
              </div>
            </div>
          )}
        </aside>
      </div>
    </>
  )
}
