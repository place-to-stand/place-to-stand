...
4. **Grid spacing tokens** (`tailwind.config.ts → theme.extend.spacing`):
...
## Design tokens
- Colors live in **both** `app/globals.css` (`--color-*` CSS vars) and `tailwind.config.ts` (hardcoded hex).
  Tailwind utilities (`text-text-muted`, `bg-bg-card`, …) resolve to the **config hex**, not the CSS vars.
  When changing a color, update `tailwind.config.ts` (and keep the matching CSS var in sync). Editing only the
  CSS var has no effect on Tailwind utility classes.
